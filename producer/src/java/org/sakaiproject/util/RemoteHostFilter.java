...
public class RemoteHostFilter implements Filter {
    ...
    // Our logger
    private static final Logger logger = LogManager.getLogger(RemoteHostFilter.class);
    ...
    public void doFilter(ServletRequest sreq, ServletResponse sres,
        ...
        for (int i = 0; i < deny.length; i++) {
            if (deny[i].matcher(host).matches()
                ...
                if (logDenied && logger.isInfoEnabled())
                    logger.info("Access denied (" + deny[i].pattern() + "): " + host + "/" + addr);
        ...
        for (int i = 0; i < allow.length; i++) {
            if (allow[i].matcher(host).matches()
                ...
                if (logAllowed && logger.isInfoEnabled())
                    logger.info("Access granted (" + allow[i].pattern() + "): " + host + "/" + addr);
                chain.doFilter(sreq, sres);
                return;
            }
        }
        ...
        if ((deny.length > 0) && (allow.length == 0)) {
            if (logAllowed && logger.isInfoEnabled())
                logger.info("Access granted (implicit): " + host + "/" + addr);
            chain.doFilter(sreq, sres);
            return;
        }
