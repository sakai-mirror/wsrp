...
public class PortletRegistrationFilterImpl implements PortletRegistrationFilter, PortletRegistrationFilterWriter
{
    ...
    private PortletRegistrationFilterImpl()
    {
        ...
        try
        {
            ...
            persistentDataObject = persistentFactory.getConsumerPortletRegistrationList();
        ...
        } catch (WSRPException e)
        {
            ...
            {
                logger.text(Logger.TRACE_MEDIUM, MN,"PortletRegistrationFilter: restore() failed!");
            }
        }
    ...
    public Iterator getAvailable(String regHandle)
    {
        ...
        if(consumerPortletRegistrationMap.containsKey(regHandle))
        {
            if (logger.isLogging(Logger.TRACE_MEDIUM))
            {
                logger.text(Logger.TRACE_MEDIUM, MN,"RegistrationHandle " + regHandle + " found in Map.");
            }
    ...
    public boolean isAvailable(String regHandle, String portletHandle)
    {
        ...
        if(consumerPortletRegistrationMap.containsKey(regHandle))
        {
            ...
            if(cpr.containsPortletHandle(portletHandle))
            {
                ...
                if (logger.isLogging(Logger.TRACE_MEDIUM))
                {
                    logger.text(Logger.TRACE_MEDIUM, MN,"PortletHandle " + portletHandle + "found for registration.");
                }
            }
        }
    ...
    */
    ...
    private void restore() throws WSRPException
    {
        ...
        {
            ConsumerPortletRegistrationImpl cpr = (ConsumerPortletRegistrationImpl) iterator.next();
            consumerPortletRegistrationMap.put(cpr.getRegistrationHandle(), cpr);
        }
