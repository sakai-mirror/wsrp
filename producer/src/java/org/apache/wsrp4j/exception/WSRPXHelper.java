...
public class WSRPXHelper
{
     ...
     * @exception WSRPException
     */
    public static void throwX(Logger logger, int logLevel, String method, int errorCode, Throwable t)
    ...
    public static void handleWSRPException(WSRPException exception) throws java.rmi.RemoteException
    {
        ...
        int errorCode = ((Integer)exceptionMapping.get(String.valueOf(exception.getErrorCode()))).intValue();
        ...
        if (errorCode == ErrorCodes.ACCESS_DENIED)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        AccessDeniedFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.ACCESS_DENIED_FAULT);
        ...
        } else if (errorCode == ErrorCodes.INCONSISTENT_PARAMETERS)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InconsistentParametersFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INCONSISTENT_PARAMETERS_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.INVALID_REGISTRATION)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InvalidRegistrationFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INVALID_REGISTRATION_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.INVALID_COOKIE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InvalidCookieFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INVALID_COOKIE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.INVALID_HANDLE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InvalidHandleFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INVALID_HANDLE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.INVALID_SESSION)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InvalidSessionFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INVALID_SESSION_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.INVALID_USER_CATEGORY)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        InvalidUserCategoryFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.INVALID_USER_CATEGORY_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.MISSING_PARAMETERS)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        MissingParametersFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.MISSING_PARAMETERS_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.OPERATION_FAILED)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        OperationFailedFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.OPERATION_FAILED_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.PORTLET_STATE_CHANGE_REQUIRED)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        PortletStateChangeRequiredFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.PORTLET_STATE_CHANGE_REQUIRED_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.UNSUPPORTED_LOCALE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        UnsupportedLocaleFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.UNSUPPORTED_LOCALE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.UNSUPPORTED_MIME_TYPE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        UnsupportedMimeTypeFault.getTypeDesc().getXmlType().getNamespaceURI(),
						Constants.UNSUPPORTED_MIME_TYPE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.UNSUPPORTED_MODE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        UnsupportedModeFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.UNSUPPORTED_MODE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
        ...
        } else if (errorCode == ErrorCodes.UNSUPPORTED_WINDOW_STATE)
        {
            ...
            try
            {
                ...
                Element element =
                    doc.createElementNS(
                        UnsupportedWindowStateFault.getTypeDesc().getXmlType().getNamespaceURI(),
                        Constants.UNSUPPORTED_WINDOW_STATE_FAULT);
                fault.clearFaultDetails();
                fault.setFaultDetail(new Element[] { element });
     ...
     */
    public static void handleWSRPFault(Logger logger, java.rmi.RemoteException wsrpFault) throws WSRPException
    {

        if (wsrpFault instanceof AccessDeniedFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1100, wsrpFault);

        } else if (wsrpFault instanceof InconsistentParametersFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1101, wsrpFault);

        } else if (wsrpFault instanceof InvalidRegistrationFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1102, wsrpFault);

        } else if (wsrpFault instanceof InvalidCookieFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1103, wsrpFault);

        } else if (wsrpFault instanceof InvalidHandleFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1104, wsrpFault);

        } else if (wsrpFault instanceof InvalidSessionFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1105, wsrpFault);

        } else if (wsrpFault instanceof InvalidUserCategoryFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1106, wsrpFault);

        } else if (wsrpFault instanceof MissingParametersFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1107, wsrpFault);

        } else if (wsrpFault instanceof OperationFailedFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1108, wsrpFault);

        } else if (wsrpFault instanceof PortletStateChangeRequiredFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1109, wsrpFault);

        } else if (wsrpFault instanceof UnsupportedLocaleFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1110, wsrpFault);

        } else if (wsrpFault instanceof UnsupportedMimeTypeFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1111, wsrpFault);

        } else if (wsrpFault instanceof UnsupportedModeFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1112, wsrpFault);

        } else if (wsrpFault instanceof UnsupportedWindowStateFault)
        {

            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1113, wsrpFault);

        } else
        {

            // its any other java.rmi.RemoteException
            WSRPXHelper.throwX(logger, Logger.ERROR, EMPTY_STRING, 1116, wsrpFault);
